...
def generate_weekly_summary(messages, start_date_str=None, end_date_str=None):
    ...
    for week_start, week_messages in sorted(weekly_messages.items()):
        ...
        participants = {msg.get('sender', 'Unknown') for msg in week_messages if msg.get('sender')}
...
def generate_conversation_intelligence(messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Build decision-ready structured intelligence for dashboard interactions and AI Q&A."""
    if not messages:
        return {
            "key_participants": [],
            "activity_patterns": {"peak_hour": None, "peak_hour_label": "N/A", "messages_at_peak": []},
            "discussion_types": [],
            "actionable_insights": [],
            "resources": {"link_count": 0, "links": [], "resource_messages": []},
            "recommendations": ["No messages found in this date range."],
            "metadata": {"total_messages": 0, "total_users": 0, "date_range": "N/A", "inactive_users": 0},
        }

    def _clean_text(text: str, max_len: int = 140) -> str:
        text = str(text or "")
        # Remove replacement chars / noisy control-like symbols from malformed exports
        text = text.replace("\ufffd", "")
        text = re.sub(r"[\u200b-\u200f\u202a-\u202e\u2066-\u2069]", "", text)
    ...
    def to_record(msg: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "timestamp": msg.get("timestamp", ""),
            "sender": msg.get("sender", "Unknown"),
            "message": _clean_text(msg.get("message", "")),
        }
    ...
    for msg in messages:
        ...
        if not text:
            continue
        lower = text.lower()
        if "messages and calls are end-to-end encrypted" in lower:
            continue
        filtered.append(msg)
    ...
    user_counter = Counter(msg.get("sender", "Unknown") for msg in filtered)
    total_users = len(user_counter)

    parsed_dates = [parse_timestamp(msg.get("timestamp", "")) for msg in filtered]
    ...
    for user, count in user_counter.most_common(5):
        pct = round((count / total_messages) * 100, 1) if total_messages else 0
        user_msgs = [to_record(m) for m in filtered if m.get("sender") == user][:8]
    ...
    peak_hour_label = f"{peak_hour:02d}:00-{peak_hour:02d}:59" if peak_hour is not None else "N/A"
    messages_at_peak = []
    if peak_hour is not None:
        for msg in filtered:
            dt = parse_timestamp(msg.get("timestamp", ""))
            if dt and dt.hour == peak_hour:
                messages_at_peak.append(to_record(msg))
        messages_at_peak = messages_at_peak[:8]

    category_rules = {
        "resource": r"https?://|\.pdf\b|\.docx?\b|\.pptx?\b|\.xlsx?\b|drive\.google|resource",
        "coordination": r"meeting|assemble|schedule|today|tomorrow|remedial|submit|call me|attendance|workshop",
        "decision": r"decided|approved|final|confirm|confirmed|will be|deadline|urgent",
        "question": r"\?|\bwho\b|\bwhat\b|\bwhen\b|\bwhere\b|\bhow\b",
    }

    category_messages: Dict[str, List[Dict[str, Any]]] = {k: [] for k in category_rules}
    for msg in filtered:
        text = str(msg.get("message", ""))
        lower = text.lower()
        for cat, pattern in category_rules.items():
            if re.search(pattern, lower):
                category_messages[cat].append(to_record(msg))

    discussion_types = []
    for cat, msgs in category_messages.items():
        discussion_types.append({
            "type": cat,
            "count": len(msgs),
            "messages": msgs[:8],
        })
    ...
    actionable_insights = []
    meeting_msgs = [to_record(m) for m in filtered if re.search(r"meeting|assemble|workshop|session", str(m.get("message", "")).lower())]
    if meeting_msgs:
        actionable_insights.append({
            "title": "Meetings or sessions detected",
            "count": len(meeting_msgs),
            "messages": meeting_msgs[:8],
        })

    decision_msgs = category_messages["decision"]
    if decision_msgs:
        actionable_insights.append({
            "title": "Decision-related messages",
            "count": len(decision_msgs),
            "messages": decision_msgs[:8],
        })
    ...
    for msg in filtered:
        ...
        if found:
            links.extend(found)
            resource_messages.append(to_record(msg))

    unique_links = list(dict.fromkeys(links))

    recommendations = []
    if key_participants:
        top_share = key_participants[0]["share_pct"]
        if top_share >= 45:
            recommendations.append(f"Top 1 user contributes {top_share}% of messages -> conversation is imbalanced.")

    inactive_threshold = 2
    inactive_users = sum(1 for _, count in user_counter.items() if count <= inactive_threshold)
    if total_users > 0:
        inactive_pct = round((inactive_users / total_users) * 100, 1)
        if inactive_pct >= 50:
            recommendations.append(f"{inactive_pct}% of users are low-activity (<= {inactive_threshold} messages).")

    decision_count = len(category_messages["decision"])
    if total_messages > 0 and (decision_count / total_messages) < 0.03:
        recommendations.append("Low number of decisions detected -> discussion is mostly informal/coordination heavy.")

    if not recommendations:
        recommendations.append("Conversation looks balanced with actionable activity present.")
    ...
    return {
        ...
        "resources": {
            ...
            "links": unique_links[:12],
            "resource_messages": resource_messages[:8],
        },
...
def build_concise_brief_report(intelligence: Dict[str, Any]) -> str:
    """Create a short, decision-ready brief report with only key details."""
    meta = intelligence.get("metadata", {})
    participants = intelligence.get("key_participants", [])
    activity = intelligence.get("activity_patterns", {})
    discussion = intelligence.get("discussion_types", [])
    actionable = intelligence.get("actionable_insights", [])
    ...
    if participants:
        top = participants[0]
        top_user_line = f"{top.get('user', 'N/A')} ({top.get('message_count', 0)} msgs, {top.get('share_pct', 0)}%)"

    peak_time = activity.get("peak_hour_label", "N/A")

    disc_map = {d.get("type", ""): d.get("count", 0) for d in discussion}
    resource_count = disc_map.get("resource", 0)
    coordination_count = disc_map.get("coordination", 0)
    decision_count = disc_map.get("decision", 0)

    meetings_count = 0
    for item in actionable:
        if "meeting" in str(item.get("title", "")).lower():
            meetings_count = item.get("count", 0)
            break

    link_count = resources.get("link_count", 0)

    rec_line = recommendations[0] if recommendations else "No recommendation available"
    ...
    lines = [
        ...
        "",
        "KEY PARTICIPANT",
        f"- Most active: {top_user_line}",
        "",
        "ACTIVITY PATTERN",
        f"- Peak time: {peak_time}",
        "",
        "DISCUSSION TYPES",
        f"- Resource sharing messages: {resource_count}",
        f"- Coordination messages: {coordination_count}",
        f"- Decision-related messages: {decision_count}",
        "",
        "ACTIONABLE INSIGHTS",
        f"- Meetings/sessions detected: {meetings_count}",
        f"- Shared links: {link_count}",
        "",
        "RECOMMENDATION",
        f"- {rec_line}",
    ]
