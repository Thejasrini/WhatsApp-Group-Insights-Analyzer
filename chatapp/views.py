...
from .summary_generator import (
    ...
    generate_conversation_intelligence,
    build_concise_brief_report,
...
def detect_response_language(requested_language, question_text):
    """Resolve response language from explicit request or question script hints."""
...
def is_help_or_system_question(question_text):
    """Detect usage/help intent so users can ask about how the platform works."""
    ...
    return any(marker in q for marker in help_markers)


def build_system_help_answer(language_code):
    """Return localized guidance about using the analytics system."""
    if language_code == "hi":
        return (
            "**WhatsApp Chat Analysis सिस्टम उपयोग गाइड**\n\n"
            "1. पहले Home पेज में WhatsApp chat file upload करें।\n"
            "2. Group चुनें, फिर Dashboard खोलें।\n"
            "3. Date range चुनकर Summary, Sentiment, Activity और Events चलाएं।\n"
            "4. Ask Questions सेक्शन में प्राकृतिक भाषा में सवाल पूछें।\n"
            "5. बेहतर उत्तर के लिए specific user, date या topic लिखें।\n\n"
            "**उदाहरण प्रश्न:**\n"
            "- इस ग्रुप में सबसे active user कौन है?\n"
            "- 12/03/2025 को क्या चर्चा हुई?\n"
            "- इस हफ्ते कितने messages आए?\n"
            "- कौन-सी files या links share हुईं?\n\n"
            "मैं सिस्टम उपयोग, features, reports और question format पर और भी मदद कर सकता हूं।"
        )

    if language_code == "ta":
        return (
            "**WhatsApp Chat Analysis அமைப்பு பயன்படுத்தும் வழிகாட்டி**\n\n"
            "1. Home பக்கத்தில் உங்கள் WhatsApp chat file-ஐ upload செய்யுங்கள்.\n"
            "2. Group தேர்வு செய்து Dashboard-க்கு செல்லுங்கள்.\n"
            "3. Date range தேர்வு செய்து Summary, Sentiment, Activity, Events இயக்குங்கள்.\n"
            "4. Ask Questions பகுதியில் நேரடி மொழியில் கேள்வி கேளுங்கள்.\n"
            "5. நல்ல பதிலுக்கு user பெயர், தேதி, topic போன்ற விவரங்களை சேர்க்கவும்.\n\n"
            "**உதாரண கேள்விகள்:**\n"
            "- இந்த குழுவில் மிகவும் active user யார்?\n"
            "- 12/03/2025 அன்று என்ன பேசப்பட்டது?\n"
            "- இந்த வாரம் எத்தனை messages வந்தது?\n"
            "- எந்த files அல்லது links share செய்யப்பட்டன?\n\n"
            "அமைப்பை எப்படி பயன்படுத்துவது, reports எப்படிப் படிப்பது, சரியான கேள்வி அமைப்பு ஆகியவற்றில் இன்னும் உதவ முடியும்."
        )

    return (
        "**WhatsApp Chat Analysis - How To Use**\n\n"
        "1. Upload your WhatsApp chat file from the Home page.\n"
        "2. Select a group and open the Dashboard.\n"
        "3. Choose a date range, then run Summary, Sentiment, Activity, or Events.\n"
        "4. Use Ask Questions for natural-language queries about your chat.\n"
        "5. For better answers, include user names, dates, or specific topics.\n\n"
        "**Example questions:**\n"
        "- Who is the most active user in this group?\n"
        "- What happened on 12/03/2025?\n"
        "- How many messages were sent this week?\n"
        "- What files or links were shared?\n\n"
        "I can also explain each dashboard section, question format, and troubleshooting steps."
    )


def localize_answer(answer_text, language_code):
    """Translate answers when needed, with safe fallback if AI translation is unavailable."""
    if language_code == "en":
        return answer_text

    language_name = SUPPORTED_RESPONSE_LANGUAGES.get(language_code, "English")
    translation_prompt = (
        f"Translate the following answer into {language_name}. Preserve markdown structure and bullets. "
        "Keep names, dates, and numbers unchanged.\n\n"
        f"Answer:\n{answer_text}"
    )
    translated = generate_with_gemini(translation_prompt)
    if translated not in {"API_ERROR", "QUOTA_EXCEEDED"}:
        return translated

    fallback_prefix = {
        "hi": "उत्तर अंग्रेजी में दिया जा रहा है क्योंकि अनुवाद सेवा अभी उपलब्ध नहीं है।",
        "ta": "மொழிபெயர்ப்பு சேவை இப்போது கிடைக்கவில்லை; பதில் ஆங்கிலத்தில் கொடுக்கப்படுகிறது.",
    }.get(language_code, "Translation is currently unavailable. Returning English answer.")
...
def parse_whatsapp(file_path):
    ...
    line_pattern = re.compile(
        r'^\[?(\d{1,2}/\d{1,2}/\d{2,4},\s*\d{1,2}:\d{2}(?::\d{2})?(?:[\u202F\u00A0 ]?[ap]m)?|\d{4}-\d{1,2}-\d{1,2},\s*\d{1,2}:\d{2}(?::\d{2})?)\]?\s*-\s*(.*)$',
        re.IGNORECASE,
    )
...
def get_group_dates(request):
    ...
    return JsonResponse({
        ...
        "end_date": end_date,
        "start_date_iso": min_date.strftime('%Y-%m-%d'),
        "end_date_iso": max_date.strftime('%Y-%m-%d')
    })
...
def group_events_analytics(request):
    ...
    if not filtered_messages:
        return JsonResponse({
            "event_counts": {'added': 0, 'left': 0, 'removed': 0, 'changed_subject': 0, 'changed_icon': 0, 'created': 0},
            "insights": {
                "most_active_day": {},
                "total_events": 0,
                "top_contributors": [],
    ...
    # Intelligence layer
    rows_sorted = sorted(rows, key=lambda x: x['dt'])
    member_change_types = {'added', 'left', 'removed'}
    setting_change_types = {'changed_subject', 'changed_icon', 'created'}

    recruiter_counts = {}
    for r in rows_sorted:
        if r['event_type'] == 'added':
            actor = (r.get('actor') or 'Unknown').strip()
            if actor and actor not in {'Unknown', 'System', 'Invite link'}:
                recruiter_counts[actor] = recruiter_counts.get(actor, 0) + 1

    top_recruiter = None
    total_joins = card_counts.get('added', 0)
    if recruiter_counts:
        recruiter_name, recruiter_count = max(recruiter_counts.items(), key=lambda kv: kv[1])
        recruiter_share = round((recruiter_count * 100.0 / total_joins), 1) if total_joins else 0
        top_recruiter = {
            'name': recruiter_name,
            'count': recruiter_count,
            'share_pct': recruiter_share,
        }

    total_exits = card_counts.get('left', 0) + card_counts.get('removed', 0)
    churn_rate = round((total_exits * 100.0 / total_joins), 1) if total_joins else 0

    subject_history = []
    for r in rows_sorted:
        if r['event_type'] == 'changed_subject':
            new_subject = (r.get('target') or '').strip()
            if not new_subject and isinstance(r.get('details'), str):
                details = r['details']
                marker = 'changed the subject to'
                if marker in details.lower():
                    new_subject = details.split('to', 1)[-1].strip().strip('"').strip("'")
            subject_history.append({
                'timestamp': r['dt'].strftime('%d/%m/%Y, %I:%M %p'),
                'actor': r.get('actor') or 'Unknown',
                'subject': new_subject or 'Updated subject',
            })
    ...
    icon_changes = [
        {
            'timestamp': r['dt'].strftime('%d/%m/%Y, %I:%M %p'),
            'actor': r.get('actor') or 'Unknown',
            'details': r.get('details') or 'Icon changed',
        }
        for r in rows_sorted if r['event_type'] == 'changed_icon'
    ]

    # Detect onboarding waves: 5+ additions within 10 minutes
    added_rows = [r for r in rows_sorted if r['event_type'] == 'added']
    onboarding_waves = []
    wave_start = 0
    for i in range(len(added_rows)):
        while wave_start <= i and (added_rows[i]['dt'] - added_rows[wave_start]['dt']).total_seconds() > 600:
            wave_start += 1
        window_size = i - wave_start + 1
        if window_size >= 5:
            current_start = added_rows[wave_start]['dt']
            current_end = added_rows[i]['dt']
            if onboarding_waves and onboarding_waves[-1]['start_iso'] == current_start.isoformat() and onboarding_waves[-1]['end_iso'] == current_end.isoformat():
                continue
            onboarding_waves.append({
                'start': current_start.strftime('%d/%m/%Y, %I:%M %p'),
                'end': current_end.strftime('%d/%m/%Y, %I:%M %p'),
                'start_iso': current_start.isoformat(),
                'end_iso': current_end.isoformat(),
                'count': window_size,
            })

    # Detect ghost periods: 30+ days with no events or chat activity
    activity_points = []
    for msg in filtered_messages:
        dt = parse_timestamp(msg.get('timestamp'))
        if dt:
            activity_points.append(dt)
    for r in rows_sorted:
        activity_points.append(r['dt'])

    activity_points = sorted(set(activity_points))
    if start_dt:
        lower_bound = start_dt
    elif activity_points:
        lower_bound = activity_points[0]
    else:
        lower_bound = None
    if end_dt:
        upper_bound = end_dt
    elif activity_points:
        upper_bound = activity_points[-1]
    else:
        upper_bound = None

    ghost_periods = []
    if lower_bound and upper_bound:
        checkpoints = [lower_bound] + activity_points + [upper_bound]
        for i in range(len(checkpoints) - 1):
            gap_days = (checkpoints[i + 1] - checkpoints[i]).days
            if gap_days >= 30:
                ghost_periods.append({
                    'start': checkpoints[i].strftime('%d/%m/%Y'),
                    'end': checkpoints[i + 1].strftime('%d/%m/%Y'),
                    'days': gap_days,
                })
    ...
    for r in reversed(rows_sorted):
        timeline.append({
            ...
            'target': r.get('target'),
            'timestamp': r['dt'].strftime('%d/%m/%Y, %I:%M %p'),
            'timestamp_iso': r['dt'].isoformat(),
            'details': r.get('details') or '',
            'category': 'member_changes' if r['event_type'] in member_change_types else 'group_settings',
        })
        ...
        "intelligence": {
            "growth_churn": {
                ...
                "recruiter_breakdown": [
                    ...
                    for name, count in sorted(recruiter_counts.items(), key=lambda x: x[1], reverse=True)
                ],
            },
            "identity_evolution": {
                "subject_history": subject_history,
                "icon_changes": icon_changes,
                "icon_change_count": len(icon_changes),
            },
            "anomalies": {
                "onboarding_waves": onboarding_waves,
...
def summarize(request):
    ...
    if not filtered_messages:
        # Return shape-compatible empty responses for summary UI views.
        # This avoids false "failed" states for valid date ranges with no data.
        if summary_type == 'weekly_summary':
            return JsonResponse({"summary_type": "weekly_summary", "weekly_summaries": []})
        elif summary_type == 'user_wise':
            return JsonResponse({"summary_type": "user_wise", "users": []})
        elif summary_type == 'user_messages':
            return JsonResponse({"summary_type": "user_messages", "user_messages": {}})
        elif summary_type == 'daily_user_messages':
            return JsonResponse({"summary_type": "daily_user_messages", "daily_summaries": []})
        elif summary_type == 'brief':
            return JsonResponse({"summary_type": "brief", "summary": "No messages found in the selected date range."})
        elif summary_type == 'total':
            return JsonResponse({"summary_type": "total", "summary": "No messages found in the selected date range."})
    ...
    try:
        if summary_type == 'total':
            ...
            intelligence = generate_conversation_intelligence(filtered_messages)
            summary = build_concise_brief_report(intelligence)
            print(f"Generated brief summary: {summary[:100]}...")
            # Ensure we always return a string
            if not isinstance(summary, str):
                summary = str(summary)
            return JsonResponse({"summary_type": "brief", "summary": summary, "intelligence": intelligence})
...
@csrf_exempt
@require_http_methods(["POST"])
def deadline_board(request):
    """Detect reminder/deadline-like messages with due hints and priority."""
    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({"error": "Invalid JSON data"}, status=400)
    ...
    if not filtered_messages:
        return JsonResponse({"deadlines": [], "summary": {"total": 0, "high_priority": 0, "with_due_hint": 0}})

    deadline_keywords = [
        'submit', 'deadline', 'due', 'exam', 'test', 'meeting', 'meet', 'assignment',
        'project', 'complete', 'bring', 'tomorrow', 'today', 'before', 'within',
        'last date', 'eod', 'urgent', 'asap', 'attend'
    ]
    high_priority_keywords = ['urgent', 'asap', 'immediately', 'must', 'within today', 'last date', 'deadline']
    due_markers = ['today', 'tomorrow', 'before', 'within', 'by', 'eod', 'am', 'pm']

    date_pattern = re.compile(r'(\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b|\b\d{1,2}:\d{2}\s?(?:am|pm)?\b)', re.IGNORECASE)
    ...
    for msg in filtered_messages:
        ...
        lowered = text.lower()
        if any(sys_term in lowered for sys_term in ['created group', 'joined using this group', 'added', 'removed', 'changed the subject']):
            continue

        if not any(k in lowered for k in deadline_keywords):
            continue

        due_hint = 'Not specified'
        date_match = date_pattern.search(text)
        if date_match:
            due_hint = date_match.group(0)
        else:
        ...
        if 'exam' in lowered or 'test' in lowered:
            kind = 'Exam/Test'
        elif 'meeting' in lowered or 'meet' in lowered:
            kind = 'Meeting'
        elif 'submit' in lowered or 'assignment' in lowered or 'project' in lowered:
            kind = 'Submission'

        priority = 'Low'
        if any(k in lowered for k in high_priority_keywords):
            priority = 'High'
        elif due_hint != 'Not specified':
            priority = 'Medium'

        clipped = text if len(text) <= 240 else (text[:237] + '...')
        extracted.append({
            "id": next_id,
            "text": clipped,
            "owner": msg.get('sender') or 'Unknown',
            "due_hint": due_hint,
            "priority": priority,
            "kind": kind,
        })
    ...
    # De-duplicate similar reminders
    deduped = []
    seen = set()
    ...
    summary = {
        "total": len(deduped),
        "high_priority": sum(1 for d in deduped if d['priority'] == 'High'),
        "with_due_hint": sum(1 for d in deduped if d['due_hint'] != 'Not specified')
    }

    return JsonResponse({"deadlines": deduped, "summary": summary})
...
def ask_question(request):
    ...
    start_date_str = data.get('start_date')
    end_date_str = data.get('end_date')
    requested_language = data.get('response_language')
    ...
    try:
        response_language = detect_response_language(requested_language, user_question)

        if is_help_or_system_question(user_question):
            return JsonResponse({
                "answer": build_system_help_answer(response_language),
                "data": {
                    "type": "assistant_help",
        ...
        result_type = str(result.get("type", "")).strip().lower().replace(" ", "_")
        ...
        if result_type == "general":
            ...
            fallback_messages = filter_messages_by_date(messages, start_date_str, end_date_str)
...
Answer:"""
            language_name = SUPPORTED_RESPONSE_LANGUAGES.get(response_language, "English")
            prompt += f"\n\nRespond in {language_name}."
            
            try:
                ai_response = generate_with_gemini(prompt)

                # Always provide an answer even when external AI is unavailable.
                if ai_response in {"API_ERROR", "QUOTA_EXCEEDED"}:
                    return JsonResponse({
                        "answer": localize_answer(generate_fallback_answer(user_question, fallback_messages), response_language),
                        "data": result,
                ...
                return JsonResponse({
                    "answer": localize_answer(ai_response, response_language),
        ...
        # For structured queries, format the response appropriately
            ...
            return JsonResponse({
                "answer": localize_answer(answer, response_language),
                "data": result,
                "language": response_language,
            })
            ...
            if topics:
                ...
                for i, topic_info in enumerate(main_topics[:10], 1):
                    ...
                    answer += f"{i}. {topic} ({frequency} mentions)\n"
                answer += "\n"
            else:
                answer = "**Topics:**\nNo clear topics were detected in the selected date range.\n\n"
        ...
        elif result_type == "phone_numbers":
            phone_numbers = result.get("phone_numbers", [])
            total_phone_numbers = result.get("total_phone_numbers", 0)

            if not phone_numbers:
                answer = "No phone numbers were found in sender names for the selected date range."
            else:
                answer = f"**Phone Numbers Found:** {total_phone_numbers}\n\n"
                for idx, row in enumerate(phone_numbers, 1):
                    phone = row.get("phone_number", "Unknown")
                    users = row.get("users", [])
                    if users:
                        answer += f"{idx}. **{phone}** - {', '.join(users)}\n"
                    else:
                        answer += f"{idx}. **{phone}**\n"
        ...
        # For other types, return the structured data with a basic response
            ...
            if result_type == "phone_numbers" or "phone_numbers" in result or "total_phone_numbers" in result:
                phone_numbers = result.get("phone_numbers", [])
                total_phone_numbers = result.get("total_phone_numbers", len(phone_numbers))
            ...
            if result_type == "general_analytics":
                ...
                answer = f"Query processed successfully. Type: {result_type or result.get('type', 'unknown')}"
...
def activity_analysis(request):
    ...
    # Keep an unsampled copy for accurate week/day splits and frontend filtering.
    full_filtered_messages = list(filtered_messages)
    ...
    if start_date_str and end_date_str:
        try:
            ...
            while current <= end_dt:
                ...
                week_msgs = [m for m in full_filtered_messages if parse_timestamp(m['timestamp']) is not None and week_start <= parse_timestamp(m['timestamp']) <= week_end]
    ...
    if include_messages:
        activity_data['messages'] = full_filtered_messages
        activity_data['all_messages'] = full_filtered_messages
