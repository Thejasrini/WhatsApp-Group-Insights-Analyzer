...
class QuestionProcessor:
    ...
    def _extract_metric_type(self, question: str) -> str:
        ...
        if any(keyword in question_lower for keyword in [
            ...
            'contact number', 'contact numbers', 'contacts', 'all numbers'
        ]):
    ...
    def classify_question(self, question: str) -> Dict[str, Any]:
        ...
        # Time-based queries should be detected before user-specific and analytics queries.
        ...
        all_users_markers = ['all users', 'all user', 'everyone', 'all members', 'entire group', 'from all users']
        asks_all_users = any(marker in question_lower for marker in all_users_markers)

        if any(pattern in question_lower for pattern in user_related_patterns) and not asks_all_users:
        ...
        # Analytics queries
        if any(keyword in question_lower for keyword in ['active', 'most', 'least', 'top', 'bottom', 'count', 'number', 'how many', 'less', 'messages', 'phone', 'contact', 'mobile']):
            return {
                'type': 'analytics',
    ...
    def _handle_analytics_query(self, messages: List[Dict], classification: Dict) -> Dict[str, Any]:
        ...
        elif metric_type == 'phone_numbers':
            return self._get_phone_numbers(messages)
    ...
    def _extract_phone_from_sender(self, sender: str) -> Optional[str]:
        """Extract a normalized phone number from a sender label, if present."""
        if not sender:
            return None

        candidates = re.findall(r'\+?\d[\d\s\-]{8,}\d', sender)
        for raw in candidates:
            digits = re.sub(r'\D', '', raw)
            if len(digits) < 10:
                continue

            # Prefer India-format display when number appears local.
            if len(digits) == 10:
    ...
    def _get_phone_numbers(self, messages: List[Dict]) -> Dict[str, Any]:
        """List unique phone numbers found in sender names within selected messages."""
        phone_to_users = defaultdict(set)

        for msg in messages:
            sender = msg.get('sender', '')
            phone = self._extract_phone_from_sender(sender)
            if phone:
                phone_to_users[phone].add(sender)

        sorted_numbers = sorted(phone_to_users.items(), key=lambda x: x[0])

        return {
            "type": "phone_numbers",
            "total_phone_numbers": len(sorted_numbers),
            "phone_numbers": [
                {
                    "phone_number": phone,
                    "users": sorted(list(users))
                }
                for phone, users in sorted_numbers
            ]
        }
    ...
    def _filter_messages_by_time(self, messages: List[Dict], time_range: Dict) -> List[Dict]:
        ...
        for msg in messages:
            ...
            if time_range['type'] == 'time_range':
                ...
                # Handle ranges crossing midnight, e.g. 18:00 to 00:00.
                if start_time <= end_time:
                    in_range = start_time <= msg_time <= end_time
                else:
                    in_range = msg_time >= start_time or msg_time <= end_time
    ...
    def _extract_time_range(self, question: str) -> Optional[Dict[str, Any]]:
        ...
        def to_24h(hour_str: str, minute_str: Optional[str], ampm: Optional[str]) -> Tuple[int, int]:
            hour = int(hour_str)
        ...
        # Support hour-only ranges like "6 PM to 12 AM" or "from 6 pm to 10 pm"
        hour_only_range_patterns = [
        ...
        for pattern in hour_only_range_patterns:
            match = re.search(pattern, question_lower)
            if match:
                sh, sa, eh, ea = match.groups()
                start_hour, start_min = to_24h(sh, None, sa)
                end_hour, end_min = to_24h(eh, None, ea)
                return {
                    'start_time': f"{start_hour:02d}:{start_min:02d}",
                    'end_time': f"{end_hour:02d}:{end_min:02d}",
                    'type': 'time_range'
